...
// synonyms in context + each pair of defs
...
function processWord($data,$line,$lnum){
    ...
    foreach($data['def1'] as $d1){
	foreach($data['def2'] as $d2){
	    $d=processDefPair($d1,$d2,$context1,$context2);
	    logExplain("graph4",false,"$d [$d1] [$d2]");
	    $c1=getCountByLabel('D1');
	    $c2=getCountByLabel('D2');
	    logExplain("graph4",false,"count(D1)=$c1 count(D2)=$c2");
	    $ma=max($c1,$c2);
	    $mi=min($c1,$c2);
	    if($ma-$mi>1)$d=-1;
	    logExplain("graph4",false,json_encode($GRAPH,JSON_PRETTY_PRINT));
	    if($dist==-1 || ($d>=0 && $d<$dist))$dist=$d;
	}
    }

    //if($found)die();
    logExplain("graph4",$lnum,$dist);
    ...
    file_put_contents("my/graph4.$DFILE.tsv",
