...
function getDefs($def,$doCorrections){
    ...
    $alldefs=[];
    foreach($defs as $def){
	if(strlen(trim($def))==0)continue;
	if(strpos($def,' or ')!==false){
    	    $data=processOrSimple($def);
    	    if(empty($data))$alldefs[]=$def;
	    else $alldefs=array_merge($alldefs,$data);
	}else $alldefs[]=$def;
    }
    //$alldefs=$defs;
...
function processOrSimple($d){
	...
	if($w=="or"){
	    ...
	    $sent=trim(implode(" ",array_merge(array_slice($base,0,count($base)-1),array_slice($words,$i+1))));
	    if(strlen($sent)>0)
		$ret[]=$sent;
	    break;
	}else {
