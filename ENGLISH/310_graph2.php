...
function processWord($data,$line,$lnum){
    ...
    $def2=trim($def2);
	    ...
	    logExplain("graph2",false,"$d [$def1] [$def2]");
	    $c1=getCountByLabel('D1');
	    $c2=getCountByLabel('D2');
	    logExplain("graph2",false,"count(D1)=$c1 count(D2)=$c2");
	    $ma=max($c1,$c2);
	    $mi=min($c1,$c2);
	    //if($ma-$mi>6)$d=-1;
	    logExplain("graph2",false,json_encode($GRAPH,JSON_PRETTY_PRINT));
	    if($dist==-1 || ($d>=0 && $d<$dist))$dist=$d;
    

    //if($found)die();
    logExplain("graph2",$lnum,$dist);
    ...
    file_put_contents("my/graph2.$DFILE.tsv",
