...
// synonyms in context (context = sum of contexts) + each pair of defs
...
function addSingleDef($def,$type,$context){
...
function processDefPair($d1,$d2,$context1,$context2){
    global $GRAPH,$stoplist;
    
    emptyGraph();
    addSingleDef($d1,"D1",$context1);
    addSingleDef($d2,"D2",$context2);
    propagateLabel("D1");
...
function processWord($data,$line,$lnum){
    ...
    logExplain("graph5",$lnum,"START");

    $context1=[];
    foreach($data['def1'] as $d1)
	$context1=array_merge($context1,getWords($d1));

    $context2=[];
    foreach($data['def2'] as $d2)
	$context2=array_merge($context2,getWords($d2));

    $context=array_merge($context1,$context2);

    $dist=-1;
    foreach($data['def1'] as $d1){
	foreach($data['def2'] as $d2){
	    $d=processDefPair($d1,$d2,$context,$context);
	    logExplain("graph5",false,"$d [$d1] [$d2]");
	    $c1=getCountByLabel('D1');
	    $c2=getCountByLabel('D2');
	    logExplain("graph5",false,"count(D1)=$c1 count(D2)=$c2");
	    $ma=max($c1,$c2);
	    $mi=min($c1,$c2);
	    if($ma-$mi>1)$d=-1;
	    logExplain("graph5",false,json_encode($GRAPH,JSON_PRETTY_PRINT));
	    if($dist==-1 || ($d>=0 && $d<$dist))$dist=$d;
	}
    }

    //if($found)die();
    logExplain("graph5",$lnum,$dist);
    ...
    file_put_contents("my/graph5.$DFILE.tsv",
